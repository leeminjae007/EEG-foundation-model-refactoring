...
def test_downstream_cosine_starts_at_full_rate_and_handles_tail_group():
    ...
    scheduler = GroupCosineScheduler(optimizer, 20 * updates, 1e-6)
    rates = []
    for _ in range(20 * updates):
        rates.append(scheduler.step())
        optimizer.step()
    assert rates[0] == [1e-5, 5e-6, 1e-4]
