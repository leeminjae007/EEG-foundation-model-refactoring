...
# SEED-V/channel_62_pos.locs reports all six positions at radius 0.51111.
# CB1 (-150 deg) bisects PO7 (-135) and O1 (-165); CB2 (+150) bisects
# PO8 (+135) and O2 (+165). Interpolate their 3D montage positions using
# those reported 2D angles instead of treating CB electrodes as PO9/PO10.
_SEED_V_POLAR_INTERPOLATION = {
    "CB1": ("PO7", "O1", 0.5),
    "CB2": ("PO8", "O2", 0.5),
}
...
def _coordinate(channel_name, montage):
    name = canonicalize_channel_name(channel_name)
    if name in _SEED_V_POLAR_INTERPOLATION:
        left, right, weight = _SEED_V_POLAR_INTERPOLATION[name]
        return (1.0 - weight) * montage[left] + weight * montage[right]
    if name in montage:
        return montage[name]
    parts = name.split("-")
    if len(parts) == 2 and parts[1] in _REFERENCES:
