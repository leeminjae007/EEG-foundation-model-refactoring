...
root_dir = '/gpfs/data/oermannlab/users/ml10266/Data/raw/physionet-mi/physionet.org/files/eegmmidb/1.0.0'
files = sorted(
    file for file in os.listdir(root_dir)
    if (
        len(file) == 4
        and file.startswith('S')
        and file[1:].isdigit()
        and os.path.isdir(os.path.join(root_dir, file))
    )
)
if files != [f'S{subject:03d}' for subject in range(1, 110)]:
    raise ValueError('PhysioNet-MI must contain subject directories S001-S109')
...
output_dir = os.environ.get(
    'PHYSIONET_MI_OUTPUT',
    '/gpfs/data/oermannlab/users/ml10266/workspace/EEG-founation-model/processed/physionet-mi/processed_average',
)
db = lmdb.open(output_dir, map_size=4614542346)
